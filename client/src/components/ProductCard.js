...
export function ProductCard({ product  }) {
  const {
    state: { kit, watchlist,compare ,history,certified},
    dispatch,
  } = useData();
  const { token } = useAuth();
...
const handleOpen = () => {
  ...
  const isInCertified = certified?.find((certifiedItem) => certifiedItem._id === _id);
  ...
  const certifiedHandler = async (e) => {
    ...
    if (!isInCertified) {
      try {
        ...
        if (success) {
          dispatch({ type: "ADD_TO_CERTIFIED", payload: product });
        ...
        <button  onClick={() => handleOpen(true)
           ...
           {open &&(
             ...
             <div className=" dialog-content-container">
              <section onClick={() => handleOpen(false) }className="dialog-content">
              <div className="bg-gray-100 mt-2 mx-2"> <img src={image} alt="equipment" className="  w-40 mx-auto  h-32 w-full p-3  "/></div>
           ...
           <h5 className=" font-semibold text-center mb-2 text-2xl  dark:text-white">{typeOfBrand}</h5>
            ...
            <div className="mx-5 mb-2 p-2 rounded-xl mt-2 bg-yellow-300">
               ...
               <span className="mb-2 ml-2 text-white font-medium text-m">I am Cerified</span>
               </div>
             ...
             <button
               ...
               onClick={(e) => {
                 e.preventDefault();
                 token ? certifiedHandler(e) : navigate("/login");
               }}
             >
              <div className="">
               <span className="material-icons-outlined  ">
               ...
               <span className="mb-2 text-m font-medium ml-2 ">Add to Certification</span>
               </div>
